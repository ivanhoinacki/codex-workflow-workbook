...
- `Datadog CLI`, `Vault MCP server`, `Vault MCP venv`, `Knowledge API URL` and `Atlassian MCP`: local paths or URLs, when they exist. The vault fields are only needed if you enable the optional `local-le-vault` server;
