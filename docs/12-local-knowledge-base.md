...
| Local knowledge search API | HTTP endpoint used by the MCP server, for example `RADAR_API_URL="http://localhost:8900"`. |
| `vault_mcp_server.py` | Downloadable template from this playbook. Exposes `query_vault` and source discovery to Codex. |
...
- vault path;
- PostgreSQL URL;
- local knowledge search API URL;
